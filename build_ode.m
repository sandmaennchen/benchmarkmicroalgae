...
% Water and thermal
if ~isfield(p,'rho_w'), p.rho_w = 1000; end                    % [kg/m³]     water density
...
if ~isfield(p,'Y_CO2'), p.Y_CO2 = 0.00039; end                 % [gCO2/gXalg] CO2 yield coefficient
if ~isfield(p,'Y_O2'), p.Y_O2 = 0.2095; end                   % [gO2/gXalg]  O2 yield coefficient
...
if ~isfield(p,'y_O2'), p.y_O2 = 0.21; end                      % [-]         O2 molar fraction in air
if ~isfield(p,'y_CO2_atm'), p.y_CO2_atm = 3.8e-4; end         % [-]         CO2 molar fraction in air (~380 ppm)
...
if ~isfield(p,'K1_ref'), p.K1_ref = 4.3e-7 * 1e3; end         % [mol/m³]    first carbonate dissociation const.
if ~isfield(p,'K2_ref'), p.K2_ref = 4.7e-11 * 1e3; end        % [mol/m³]    second carbonate dissociation const.
