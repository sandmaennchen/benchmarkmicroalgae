...
% Geometry
if ~isfield(p,'L'), p.L = 40; end
if ~isfield(p,'W'), p.W = 1; end
if ~isfield(p,'A'), p.A = p.L * p.W; end
if ~isfield(p,'rsump'), p.rsump = 0.4; end
if ~isfield(p,'hsump'), p.hsump = 1.12; end
if ~isfield(p,'Asump'), p.Asump = pi * p.rsump^2; end
if ~isfield(p,'Vsump'), p.Vsump = p.Asump * p.hsump; end

% Water and thermal
if ~isfield(p,'rho_w'), p.rho_w = 1000; end
if ~isfield(p,'Cp_w'), p.Cp_w = 4182; end
if ~isfield(p,'alpha_rad'), p.alpha_rad = 0.7; end
if ~isfield(p,'sigma'), p.sigma = 5.67e-8; end
if ~isfield(p,'eps_w'), p.eps_w = 0.97; end
if ~isfield(p,'x_liner'), p.x_liner = 0.003; end
if ~isfield(p,'K_liner'), p.K_liner = 0.35; end
if ~isfield(p,'x_ug'), p.x_ug = 0.20; end
if ~isfield(p,'K_ug'), p.K_ug = 1.0; end
if ~isfield(p,'T_g'), p.T_g = 20; end
if ~isfield(p,'k_m'), p.k_m = 0.01; end
if ~isfield(p,'h_c'), p.h_c = 10; end
if ~isfield(p,'P_mix'), p.P_mix = 200; end
if ~isfield(p,'UA_HX'), p.UA_HX = 500; end
if ~isfield(p,'T_in_medium'), p.T_in_medium = 20; end
if ~isfield(p,'Lpw'), p.Lpw = 2; end

% Biology
if ~isfield(p,'mu_max'), p.mu_max = 0.8/86400; end
if ~isfield(p,'Ka'), p.Ka = 0.2; end
if ~isfield(p,'Ik'), p.Ik = 200; end
if ~isfield(p,'n_hill'), p.n_hill = 1; end
if ~isfield(p,'T_min'), p.T_min = 5; end
if ~isfield(p,'T_opt'), p.T_opt = 30; end
if ~isfield(p,'T_max'), p.T_max = 40; end
if ~isfield(p,'pH_min'), p.pH_min = 6; end
if ~isfield(p,'pH_opt'), p.pH_opt = 8; end
if ~isfield(p,'pH_max'), p.pH_max = 10; end
if ~isfield(p,'DO_max'), p.DO_max = 383.21; end
if ~isfield(p,'m_DO'), p.m_DO = 2; end
if ~isfield(p,'eta_X'), p.eta_X = 0.6; end
if ~isfield(p,'Y_CO2'), p.Y_CO2 = 1.92; end
if ~isfield(p,'Y_O2'), p.Y_O2 = 1.41; end
if ~isfield(p,'M_CO2'), p.M_CO2 = 44; end
if ~isfield(p,'M_O2'), p.M_O2 = 32; end
if ~isfield(p,'m_min'), p.m_min = 0.02/86400; end
if ~isfield(p,'k_resp_I'), p.k_resp_I = 0.5; end
if ~isfield(p,'Q10'), p.Q10 = 2.0; end

% Chemistry and transfer
if ~isfield(p,'T_ref'), p.T_ref = 298.15; end
if ~isfield(p,'R_gas'), p.R_gas = 8.314; end
if ~isfield(p,'p_atm'), p.p_atm = 1.0; end
if ~isfield(p,'y_O2'), p.y_O2 = 0.21; end
if ~isfield(p,'y_CO2_atm'), p.y_CO2_atm = 3.8e-4; end
if ~isfield(p,'y_CO2_inj'), p.y_CO2_inj = 1.0; end
if ~isfield(p,'KH_O2_ref'), p.KH_O2_ref = 1.3e-3 * 1e3; end
if ~isfield(p,'KH_CO2_ref'), p.KH_CO2_ref = 3.4e-2 * 1e3; end
if ~isfield(p,'C_O2'), p.C_O2 = 1700; end
if ~isfield(p,'C_CO2'), p.C_CO2 = 2400; end
if ~isfield(p,'K1_ref'), p.K1_ref = 4.3e-7 * 1e3; end
if ~isfield(p,'K2_ref'), p.K2_ref = 4.7e-11 * 1e3; end
if ~isfield(p,'KW_ref'), p.KW_ref = 1e-14 * 1e6; end
if ~isfield(p,'dH_K1'), p.dH_K1 = -7500; end
if ~isfield(p,'dH_K2'), p.dH_K2 = -15000; end
if ~isfield(p,'dH_KW'), p.dH_KW = 55900; end
if ~isfield(p,'alpha_CO2'), p.alpha_CO2 = 0.05; end
if ~isfield(p,'beta_CO2'), p.beta_CO2 = 0.7; end
if ~isfield(p,'alpha_O2'), p.alpha_O2 = 0.04; end
if ~isfield(p,'beta_O2'), p.beta_O2 = 0.7; end
if ~isfield(p,'k_strip_CO2_O2'), p.k_strip_CO2_O2 = 0.05; end
if ~isfield(p,'k_strip_O2_CO2'), p.k_strip_O2_CO2 = 0.02; end
if ~isfield(p,'k_atm_CO2'), p.k_atm_CO2 = 1e-5; end
if ~isfield(p,'k_atm_O2'), p.k_atm_O2 = 2e-5; end
if ~isfield(p,'k_pw_CO2'), p.k_pw_CO2 = 1e-5; end
if ~isfield(p,'k_pw_O2'), p.k_pw_O2 = 2e-5; end
if ~isfield(p,'DIC_in'), p.DIC_in = 2; end
if ~isfield(p,'Cat_in'), p.Cat_in = 5; end
